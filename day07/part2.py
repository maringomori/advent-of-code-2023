...
def analize_hand(hand):
    ...
    sorted_hands = sorted(hand_dict, key=hand_dict.get, reverse=True)
    
    if "J" in hand_dict.keys():
        if sorted_hands[0] == "J":
            try: 
                hand_dict[sorted_hands[1]] += hand_dict["J"]
                del hand_dict["J"]
            except:
                pass
        else:
            hand_dict[sorted_hands[0]] += hand_dict["J"]
            del hand_dict["J"]
...
cards_order = {"A": 13, "K": 12, "Q": 11, "J": 0, "T": 9, "9": 8, "8": 7, "7": 6, "6": 5, "5": 4, "4": 3, "3": 2, "2": 1}
